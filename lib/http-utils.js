var swaggerHttpUtils = function (utils) {
  this.utils = utils;
  /*******************PRIVATE METHODS*********************/
  var addAuthentication = function(request, options){
    if(options.authentication !== undefined){
      ...
      if(options.authentication.token !== undefined){
        request.setRequestHeader('Authentication', 'Token '+options.authentication.token);
      }
    }
    return request;
  };
  ...
  this.call = function (callOptions) {
    ...
    */
    ...
    if(options.method === 'GET'){
      ...
      request = addAuthentication(request, options);
      options.parameters = {};
    }else{
  ...
  /**
   * Encharged of getting the definition of the params and returning the actually
   * list of params
  ...
  var processParams = function(params){
    ...
    for (var i = 0; i < params.length; i++) {
      param = params[i];
      // if is not required and doesn't have any value we do nothing
      if(param.required === false && param.value === undefined){
        continue;
      }else{
